...
int main()
{
	...
	int serial = 0;// number of entries in the history file
	//=========================================================================================
		...
		if(is_present(user_input_array, argument_numbers, "|"))
		{
			...
			if(pipe(fd) >=0 )
			{
				...
				else if(child1 == 0)
				{
					...
					//fd[1] -  will be the fd for the write end of pipe Returns : 0 on Success,-1 on error.
					close(fd[0]);
					dup2(fd[1], STDOUT_FILENO); //creates the copy of the file discripter
					//arg1 - old file disdripter
					//arg2 - new file discripter which is used by dup2 to create a copy
					close(fd[1]);
	...
	print_message("This Mini Shell is created by Aditya",' ');
	printf("\n");
	RESET
...
int count_argument_numbers(char* user_input)
{
	...
	for(i; i<strlen(user_input); i++)
	{
		...
		if(user_input[i] == '\"' || user_input[i] == '\'')//skiping the quotes
		{
			int j;
...
char** separate_user_input(char* user_input, int n) //(parsing the string)
{
	//Allocate memory.
	...
	if(user_input[0] == '>' && user_input[1]=='>')
	{
		...
		i+=2; //here i count mentions the double redirection
		j++;
	}
	//Start the loop.
	for(i; i<strlen(user_input); i++)
...
char* get_program_path(char* command, char** all_paths) //command = "cat" paths = all_paths
{
